...
<#if pracowniks??>
	    <div id="_title_div">
		    ...
		    <@spring.message code="label.pracowniks" var="entity_label_plural"/>
			...
			<table>
				...
				<tbody>
					<#list pracowniks as pracownik>
						<tr>
							<#escape x as x?html>
								<td>${pracownik.id}</td><td>${pracownik.username}</td>
							</#escape>  
                            <@spring.url value="/pracownik/${pracownik.id}/form" var="update_form_url"/>
                            <@spring.url value="/static/images/update.png" var="update_image_url"/>							
                            <@spring.url value="/pracownik/${pracownik.id}" var="show_url"/>
                            <@spring.url value="/static/images/show.png" var="show_image_url"/>							
                            <@spring.url value="/pracownik/${pracownik.id}" var="delete_form_url"/>
				...
				<tfoot>
					<tr>
						<td colspan="5">
                        <@spring.url value="/pracownik/form" var="create_form_url"/>
