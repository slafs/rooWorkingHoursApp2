...
<#if projekts??>
	    <div id="_title_div">
		    ...
		    <@spring.message code="label.projekts" var="entity_label_plural"/>
			...
			<table>
				...
				<tbody>
					<#list projekts as projekt>
						<tr>
							<#escape x as x?html>
								<td>${projekt.id}</td><td>${projekt.name}</td>
							</#escape>  
                            <@spring.url value="/projekt/${projekt.id}/form" var="update_form_url"/>
                            <@spring.url value="/static/images/update.png" var="update_image_url"/>							
                            <@spring.url value="/projekt/${projekt.id}" var="show_url"/>
                            <@spring.url value="/static/images/show.png" var="show_image_url"/>							
                            <@spring.url value="/projekt/${projekt.id}" var="delete_form_url"/>
				...
				<tfoot>
					<tr>
						<td colspan="5">
                        <@spring.url value="/projekt/form" var="create_form_url"/>
